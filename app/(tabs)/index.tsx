...
export default function PlayersScreen() {
  const { players, isLoadingPlayers, addPlayer, updatePlayer, deletePlayer } = usePlayers();
  ...
  const savePlayer = () => {
    ...
    if (!nextPlayer.firstName || !nextPlayer.lastName || !nextPlayer.birthdayDate) {
      Alert.alert('Missing information', 'First name, last name and birthday date are required.');
      return;
    }

    if (!isValidBirthdayDate(nextPlayer.birthdayDate)) {
      Alert.alert('Invalid birthday date', 'Use YYYY-MM-DD and choose a real date.');
      return;
    }

    if (nextPlayer.photoUri && !isValidPhotoUrl(nextPlayer.photoUri)) {
      Alert.alert('Invalid photo URL', 'Photo must be a valid http or https URL.');
      return;
    }
  ...
  const confirmDeletePlayer = (player: Player) => {
    Alert.alert('Delete player', `Delete ${player.firstName} ${player.lastName}?`, [
  ...
  return (
    <SafeAreaView style={styles.screen}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <AppCard tone={players.length >= 2 ? 'default' : 'gold'} style={styles.notice}>
          <MaterialIcons
            name={players.length >= 2 ? 'check-circle' : 'info-outline'}
            size={20}
            color={players.length >= 2 ? AppTheme.colors.success : AppTheme.colors.primary}
          />
        ...
        {isLoadingPlayers ? (
          <AppCard style={styles.emptyState}>
            <MaterialIcons name="storage" size={32} color={AppTheme.colors.primary} />
            <Text style={styles.emptyTitle}>Loading saved players</Text>
            <Text style={styles.emptyText}>Your local player profiles are being restored.</Text>
          </AppCard>
        ) : players.length === 0 ? (
          <AppCard style={styles.emptyState}>
            <PlayerAvatar size={58} />
            <Text style={styles.emptyTitle}>No players yet</Text>
            <Text style={styles.emptyText}>Create your first players to prepare the match setup.</Text>
            <AppButton title="Create player" icon="add" onPress={openCreateForm} />
          </AppCard>
        ) : (
          players.map((player) => (
            <Pressable key={player.id} onPress={() => openEditForm(player)}>
              <AppCard style={styles.playerCard}>
                ...
                <View style={styles.playerInfo}>
                  ...
                  <Text style={styles.playerMeta}>Birthday: {player.birthdayDate}</Text>
                </View>
                <MaterialIcons name="edit" size={22} color={AppTheme.colors.primary} />
              </AppCard>
            </Pressable>
          ))
        )}
      ...
      <AppModal
        ...
        <ScrollView contentContainerStyle={styles.formContent} keyboardShouldPersistTaps="handled">
          <View style={styles.avatarPreview}>
            ...
            <Text style={styles.avatarText}>Photo is optional. Leave empty to use the default icon.</Text>
          </View>
          ...
          <SegmentedControl<Handedness>
            ...
            onChange={(handedness) => setForm((current) => ({ ...current, handedness }))}
            options={[
          ...
          <AppInput
            ...
            onChangeText={(birthdayDate) => setForm((current) => ({ ...current, birthdayDate }))}
          />
          ...
          <AppInput
            ...
            value={form.photoUri}
            onChangeText={(photoUri) => setForm((current) => ({ ...current, photoUri }))}
          />
          ...
          <View style={styles.textAreaGroup}>
            ...
            <TextInput
              ...
              onChangeText={(strengths) => setForm((current) => ({ ...current, strengths }))}
            />
          </View>
          ...
          <View style={styles.textAreaGroup}>
            ...
            <TextInput
              ...
              onChangeText={(weaknesses) => setForm((current) => ({ ...current, weaknesses }))}
            />
          </View>
          ...
          <View style={styles.formActions}>
            ...
            <AppButton title={editingPlayer ? 'Save changes' : 'Create player'} icon="check" onPress={savePlayer} />
          </View>
        </ScrollView>
      </AppModal>
    </SafeAreaView>
